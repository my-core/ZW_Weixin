...
namespace Weixin.MP.CommonAPIs
{
    ...
    public partial class CommonApi
    {
        ...
        /// <returns></returns>
        public static AccessTokenResult GetToken(string appid, string secret, string grant_type)
        {
            if (grant_type == "")
            {
            ...
            AccessTokenResult result = Get.GetJson<AccessTokenResult>(url,null);
            return result;
        }
        ...
        public static WeixinUserInfoResult GetUserInfo(string accessToken, string openId)
        {
            var url = string.Format("http://api.weixin.qq.com/cgi-bin/user/info?access_token={0}&openid={1}",
                                    accessToken, openId);
            WeixinUserInfoResult result = Get.GetJson<WeixinUserInfoResult>(url,null);
            return result;
        }
        ...
        public static UploadMediaFileResult UploadMediaFile(string accessToken, UploadMediaFileType type, string fileName)
        {
            ...
            UploadMediaFileResult result = Post.PostGetJson<UploadMediaFileResult>(url, cookieContainer, fileStream,null);
            return result;
        }
    }
}
